...
graph TB
    ...
    subgraph BACKEND["后端核心层 — Rust Backend"]
        direction LR
        ENGINE["翻译引擎<br/>并行翻译编排<br/>语言自动检测<br/>结果聚合与排序"]
        PLUGINMGR["插件管理器<br/>Plugin Trait 注册<br/>插件生命周期<br/>配置校验与测试"]
        SERVICEMGR["服务管理器<br/>服务 CRUD 操作<br/>启用/禁用控制<br/>多服务实例管理"]
        HOTKEYMGR["快捷键管理器<br/>全局热键注册<br/>快捷键重绑定<br/>Win/Mac/Linux"]
        CONFIGMGR["配置管理器<br/>全局配置读写<br/>导入/导出/重置<br/>默认值管理"]
    end

    subgraph TRAIT["插件接口层 — Plugin Trait"]
        direction LR
        GOOGLE["Google 翻译<br/>免费 Web API<br/>语言自动检测"]
        DEEPL["DeepL 适配器<br/>REST API<br/>术语表支持"]
        OPENAI["OpenAI 兼容<br/>OpenAI / Ollama<br/>自定义 Prompt"]
        GENERIC["通用 HTTP<br/>自定义端点<br/>请求/响应模板"]
        DICT["本地词典<br/>离线词库<br/>正则匹配替换"]
    end

    subgraph INFRA["基础设施层"]
        direction LR
        SELECTION["文本选择捕获<br/>selection crate<br/>剪贴板回退"]
        STORE["配置持久化<br/>SQLite + JSON<br/>Keychain 加密"]
        HTTP["HTTP 客户端<br/>reqwest 异步<br/>TLS/代理"]
        DB["数据库 SQLite<br/>服务/历史/配置"]
    end
...
采用 **Trait-based Plugin Architecture**，每个翻译源实现统一的 `Translator` trait。Edition 2024 支持原生 `async fn` in traits，无需 `#[async_trait]` 宏。
...
pub trait Translator: Send + Sync {
    ...
    /// 验证配置是否有效
    async fn validate_config(&self, config: &PluginConfig) -> Result<bool>;
    /// 执行翻译
    async fn translate(
        &self,
        text: &str,
...
graph TB
    ...
    CARD --> ACTIONS["ActionButtons.vue<br/>复制/朗读"]
