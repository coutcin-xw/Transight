...
| UI 样式 | CSS 自定义属性 + 组件 scoped 样式 | 无第三方 UI 库，轻量可控 |
| 状态管理 | Pinia | Vue 3 官方状态管理 |
| 构建工具 | Vite | 快速 HMR、配置简洁 |
| 后端 | Rust (edition 2024) | 高性能、类型安全、与 Tauri 深度集成 |
| 文本捕获 | selection crate（自研） | 跨平台文本选择捕获 (X11/Wayland/Accessibility/UIA) |
| 配置存储 | JSON + Arc<RwLock<ConfigStore>> | 轻量配置持久化 |
| 中文排版 | Inter + Noto Sans SC 自带 + 系统回落 | 跨平台中英文一致渲染 |
...
graph TB
    subgraph FRONTEND["前端层 — Vue 3 + TypeScript + Pinia"]
        direction LR
        POPUP["翻译弹窗<br/>350×540 可 resize<br/>语言选择器 / 输入框<br/>多翻译源结果卡片"]
        SETTINGS["设置窗口<br/>700×540 侧边栏导航<br/>常规 / 服务管理 / 快捷键<br/>暗色模式 / 导入导出"]
        COMPONENTS["通用组件<br/>TitleBar / ResizeHandles<br/>TranslationCard<br/>ServiceManager"]
        PINIA["Pinia 状态管理<br/>translation store<br/>主题 / 快捷键录制"]
    end
...
| 适配器 | 类型 | 配置项 |
|--------|------|--------|
| Google Translate | Traditional | 无 |
| DeepL | Traditional | `api_key`, `api_url` |
| OpenAI 兼容 | LLM | `api_key`, `api_url`, `model`, `prompt_template`, `response_path` |
...
```json
{
  ...
  "services": [
    { "id": "svc-google", "plugin_id": "google-translate", "name": "Google 翻译", "enabled": true, "config": {} },
    { "id": "svc-deepl", "plugin_id": "deepl", "name": "DeepL", "enabled": false, "config": { "api_key": "" } },
    { "id": "svc-llm", "plugin_id": "openai-compat", "name": "LLM 翻译", "enabled": false, "config": { "api_url": "https://api.openai.com/v1/chat/completions", "model": "gpt-4o-mini" } }
  ],
  "shortcuts": {
...
```
~/.config/transight/
  config.json           # 全局配置 (版本/通用设置/服务列表/快捷键)
```

`ConfigStore` = `Arc<RwLock<Config>>`，启动时加载，变更即保存。
...
pub struct GeneralConfig {
    pub default_source_lang: String,  // "auto"
    pub default_target_lang: String,  // "zh"
...
pub struct ServiceConfig {
    pub id: String,        // "svc-google"
    pub plugin_id: String, // "google-translate"
    pub name: String,      // "Google 翻译"
    pub enabled: bool,
    pub config: Value,     // JSON (api_key/api_url/model/prompt_template/response_path)
}

pub struct ShortcutConfig {
    pub translate_selected: String,  // "Ctrl+Alt+Q"
    pub close_window: String,        // "Escape"
}
```
